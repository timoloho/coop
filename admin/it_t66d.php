    ...
    <main role="main" class="container-fluid">
			<form method="POST" enctype="multipart/form-data">
				...
				<div class="form-group">
					<select class="custom-select w-15 float-left bg-light mr-1" name="sort">
						<option value="<?=$_POST[sort]?>"><? if (!empty($_POST['sort'])){
							echo $_POST['sort'];
						}else {
							echo "Kõik kauplused";
						} ?>
						...
						<label for="nimi"></label>
							<select class="custom-select shadow w-15 float-left bg-light mr-1" name="syndmus">
								<option value="<?=$_POST['syndmus']?>"><? if (!empty($_POST['syndmus'])){
							echo $_POST['syndmus'];
						}else {	
							echo "Kõik sündmused";
						} ?></option>
				...
				<select class="custom-select shadow w-15 bg-light font-weight-bold float-left mr-1" name="tehteg">
					<option class="font-weight-bold" value="<?=$_POST['tehteg']?>"><? if (!empty($_POST['tehteg']) && $_POST['tehteg'] == 1){
								echo "Kuva tehtud";
							} else if (!empty($_POST['tehteg'])){
								echo $_POST['tehteg'];
							} else {	
								echo "Kuva kõik";
							} ?>
					<option value="">Kuva kõik</option>
					<option value="Tegemata">Kuva tegemata</option>
					<option value="1">Kuva tehtud</option>	
				</select>
			...
			<?php
			include_once("tabelid/it_tabel.php");?>
			</div>
		<div id="reload">
	...
	$(document).ready(function(){
		...
		$('#save').click(function(){
			...
			$.ajax({
				url : 'php/update.php',
				method : 'post',
				data : {kirjeldus : kirjeldus , markused : markused , id : id},
		...
		$('#kinnita').click(function(){
			...
			$.ajax({
				...
				data : {kirjeldus : kirjeldus , markused : markused , id : id},
				success : function(response){
					$('#'+id).children('td[data-target=kinnita]').val(id);
					$('#'+id).children('td[data-target=kirjeldus]').text(kirjeldus);
					$('#'+id).children('td[data-target=markused]').text(markused);
					location.reload();
				}
