<?php
	...
	if(isset($_POST["save"])) {
		if(!empty($_FILES["piltupdate"]["name"])){
		  $check = getimagesize($_FILES["piltupdate"]["tmp_name"]);
			  if($check !== false) {
				  compressImage($_FILES['piltupdate']['tmp_name'],$target_file,60);

			  } else {
					echo "Valitud fail ei ole pildi formaati";
					}
		} 
	}
	...
	function compressImage2($source, $destination, $quality) {
	  ...
	  $exif = exif_read_data($_FILES['piltupdate']['tmp_name']);
